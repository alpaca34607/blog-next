 ...
 * 
 * 使用方法：
 * 1. 轉換所有明文密碼：npx tsx scripts/hash-passwords.ts
 * 2. 轉換特定用戶：npx tsx scripts/hash-passwords.ts --email admin-test@gmail.com
 */
...
async function hashPasswords(email?: string) {
  try {
    console.log('開始轉換密碼...\n')
    ...
    const usersToProcess = users.filter(Boolean) as Array<NonNullable<typeof users[0]>>
