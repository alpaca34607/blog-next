...
const SortableColumnItem=( {
  ...
  return (<div ref= {
    ...
    onChange= {
      (e)=> onUpdate(index, e.target.value)
    }
...
const TableModal=( {
  ...
  const [formData,
  setFormData]=useState<Partial<CustomTable>>( {
      name: "",
      description: "",
      columns: [],
      show_search: true,
  ...
  const handleSubmit=(e: React.FormEvent)=> {
    ...
    const cleanedColumns=(formData.columns || []) .map((c)=> String(c || "").trim()) .filter(Boolean);
  ...
  const handleDragEnd=(event: DragEndEvent)=> {
    ...
    const oldIndex=typeof active.id==="number"? active.id : Number(active.id);
    const newIndex=typeof over.id==="number"? over.id : Number(over.id);

    if (Number.isNaN(oldIndex) || Number.isNaN(newIndex) || oldIndex < 0 || newIndex < 0 || oldIndex >=columns.length || newIndex >=columns.length) {
      return;
    }
  ...
  return (<div className= {
    ...
    > {
      (formData.columns || []).map((col, index)=> (<SortableColumnItem key= {
            index
          }
