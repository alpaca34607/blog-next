...
interface ImageUploaderProps {
  ...
  showUrlInput?: boolean; // 是否顯示貼上網址欄位，預設 true
...
type UploadApiResponseData=UploadFileResult | {
  files: UploadFileResult[]
}

;

/** 檢查是否為有效的 http/https 網址 */
...
const ImageUploader=( {
  ...
  const uploadImages=async (files: File[]): Promise<string[]>=> {
    ...
    if (data && typeof data==="object") {
      ...
      if (Array.isArray(data.files)) {
        const urls=data.files .map((x: any)=> x?.url) .filter((x: any)=> typeof x==="string");
  ...
  const handleFileChange=async (e: React.ChangeEvent<HTMLInputElement>)=> {
    ...
    try {
      ...
      if (multiple) {
        if ( !onMultipleUpload) {
          console.warn("ImageUploader: multiple 為 true 但未提供 onMultipleUpload 回調函數"
          );
          return;
        }
  ...
  return (<div className= {
    ...
    /> {
      ...
      {
      showUrlInput && (<div className= {
          styles.urlInputBlock
        }
