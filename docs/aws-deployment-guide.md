...
建立完成後，記下以下資訊：
- **端點**：例如 `watchsense-db.xxxxx.us-east-1.rds.amazonaws.com`
- **連接埠**：3306
- **資料庫名稱**：`watchsense_prod`
- **使用者名稱**：`admin`
- **密碼**：您設定的密碼
...
```bash
# 匯出資料庫結構和資料
mysqldump -h <現有資料庫主機> -u <使用者名稱> -p <資料庫名稱> > watchsense_backup.sql

# 或只匯出結構（不含資料）
mysqldump -h <現有資料庫主機> -u <使用者名稱> -p --no-data <資料庫名稱> > watchsense_schema.sql
```
...
```env
DATABASE_URL="mysql://admin:YOUR_PASSWORD@watchsense-db.xxxxx.us-east-1.rds.amazonaws.com:3306/watchsense_prod"
```
...
3. **匯入現有資料**（如果需要）：
```bash
mysql -h <RDS端點> -u admin -p watchsense_prod < watchsense_backup.sql
```
...
const nextConfig: NextConfig = {
  images: {
    remotePatterns: [
      {
        hostname: 'watchsense-prod.s3.us-east-1.amazonaws.com',
        // 或使用 CloudFront 域名
...
A: 使用 AWS CLI：
```bash
aws s3 sync s3://old-bucket s3://watchsense-prod
```
